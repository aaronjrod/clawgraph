...
class TestDeadEndCascade:
    ...
    def test_failed_producer_cascades_stalled_consumers(self):
        ...
        assert "consumer" in result.get("completed_nodes", [])

    def test_failed_producer_does_not_cascade_unrelated_nodes(self):
        ...
        assert "unrelated_consumer" in result.get("stalled_queue", [])

    def test_cascade_emits_timeline_event(self):
        ...
        assert len(dead_end_events) >= 1, "Should have a DEAD_END timeline event"
        assert dead_end_events[0]["node_id"] == "doomed_consumer"

    def test_cascade_marks_nodes_completed(self):
