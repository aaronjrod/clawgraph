...
- **Synthesized Failure**: If a python exception occurs, the Orchestrator catches it and emits a `FAILED` signal.
- **Payload**: These synthesized failures set `orchestrator_synthesized: true` and include the full traceback. This signals to the Super-Orchestrator that the error metadata was not generated by node reasoning and should be treated as a system crash.
